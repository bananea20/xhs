...
var Editor = require(25);
var cache = require(12);
var _ = require(13);
...
var MainTab = require(30);
var SettingsTab = require(31);
var AboutTab = require(29);
var cache = require(12);
...
module.exports = {
  ...
  mounted: function() {
    var self = this;
    this.loadScripts().then(function(items) { self.items = items; });
  },
  computed: {
  ...
  methods: {
    loadScripts: function(opts) {
      this.loading = true;
      var self = this;
      return greasyfork.getScripts(opts).then(function(response) {
        self.loading = false;
        return response;
      });
    },
    installScript: function(item) {
      greasyfork.getDetailScript(item.detailUrl).then(function(info) {
        location.href = info.installURL;
      });
    },
    searchSubmit: function() {
      ...
      this.items = [];
      this.currentPage = 1;
      const opts = {search: this.searchInput};
      this.loadScripts(opts).then(function(result) { self.items = result; });
    },
    go: function(page) {
      ...
      this.loadScripts(request).then(function(items) {
        self.items = items;
      });
    },
  },
...
module.exports = {
  ...
  mounted: function() {
    ...
    if (params) {
      ...
      parserName = params.get('service');
    } else {
      location.href = '#main/Library';
      return;
    }
    if (parserName === 'greasyfork') {
      parser = greasyfork;
    }

    scriptUrl = parser.endpoints.host + params.get('path');
    var self = this;
    parser.getDetailScript(scriptUrl).then(function(scr) {
      self.script = scr;
      self.loading = false;
    });
  },
...
var PARSER_NAME = 'greasyfork';

var AVAILABLE_LANGUAGES = ['ar','bg','cs','da', 'de','el','en','eo','es','fi','fr','fr-CA','he','hu','id','it','ja','ko','nb','nl','pl','pt-BR','ro','ru','sk','sr','sv','th','tr','uk','vi','zh-CN','zh-TW'];

var endpoints = {
  host: 'https://greasyfork.org',
  scripts: function (options) {
    if (!options) { options = {}; }
    var params = new URLSearchParams();
    if (!options.lang) {
      options.lang = navigator.language || navigator.userLanguage;
      if (options.lang.indexOf('-') !== -1) {
        options.lang = options.lang.split('-')[0];
        if (!AVAILABLE_LANGUAGES.includes(options.lang)) {
          options.lang = 'en';
        }
      }
    }
    ...
    var url = 'https://greasyfork.org/' + options.lang + '/scripts';
    if (Array.from(params.entries()).length) { url += '?' + params.toString(); }

    return url;
  },
};

function parseHtmlFragment(str, location) {
...
function request(method, url) {
  return new Promise(function (resolve, reject) {
    ...
    xhr.onreadystatechange = function () {
      ...
      return xhr.status == 200 ? resolve(xhr.responseText) : reject();
    };
...
module.exports = {
  ...
  getScripts: function (options) {
    var url = endpoints.scripts(options);
    return request('GET', url).then(function (res) {
      var elements = parseHtmlFragment(res).querySelectorAll('li[data-script-id]');
      return Array.from(elements).map(function (element) {
        var linkEl = element.querySelector('a[href*="/scripts/"]');
        if (!linkEl) {
          return null;
        }
        ...
        var payload = {
          name: name,
          host: endpoints.host,
          url: 'service=' + PARSER_NAME + '&path=' + encodeURIComponent(href),
          detailUrl: endpoints.host + href,
          description: desc
        };
        return payload;
      }).filter(function (item) {
  ...
  getDetailScript: function (url) {
    return request('GET', url).then(function (body) {
      var page = parseHtmlFragment(body);
      var h2Name = page.querySelector('h2');
      var name = h2Name ? h2Name.textContent : '';

      var divDesc = page.querySelector('#script-description');
      var desc = divDesc ? divDesc.textContent : '';

      var labels = page.querySelectorAll('#script-stats dt');
      var values = page.querySelectorAll('#script-stats dd');
      var stats = Array.from(labels).map(function (label, i) {
        var value = values[i] ? values[i].outerHTML : '';
        if (value) {
          value = value
            .replace(/img src="\//g, 'img src="' + endpoints.host + '/');
        }
        return {
      ...
      var divAuthorDesc = page.querySelector('#additional-info');
      var authorDesc = '';
      if (divAuthorDesc) {
        var screenshots = divAuthorDesc.querySelector('.user-screenshots');
        screenshots && screenshots.remove();
        authorDesc = divAuthorDesc.textContent;
      }
      var aInstallUrl = page.querySelector('a.install-link');
      var installUrl = aInstallUrl ? endpoints.host + aInstallUrl.getAttribute('href') : '';
      return {
        name: name,
        description: desc,
